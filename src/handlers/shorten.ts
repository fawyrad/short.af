...
export const shorten = async (request: Request, env: Env, ctx: ExecutionContext): Promise<Response> => {
	...
	if (await checkExists(env, shortcut.from)) {
		return new Response(JSON.stringify({ status: 'error', message: 'shortcut already exists' }), {
			status: 422,
			headers: { 'content-type': 'application/json' },
		})
	}
	...
	if (
		shortcut.from.length < 3 ||
		['api', 'expand', 'shorten', '.htaccess', 'wp-login.php'].some((from) => shortcut.from.startsWith(from))
	) {
		return new Response(JSON.stringify({ status: 'error', message: 'shortcut not allowed' }), {
			status: 403,
			headers: { 'content-type': 'application/json' },
		})
	}
	...
	if (env.GOOGLE_SAFE_BROWSING_KEY && !(await checkSafeBrowsing(env, shortcut.dest))) {
		return new Response(JSON.stringify({ status: 'error', message: 'destination is unsafe' }), {
			status: 403,
			headers: { 'content-type': 'application/json' },
		})
	}
	...
	try {
		...
		return new Response(JSON.stringify({ status: 'success', ...shortcut }), {
			status: 201,
			headers: { 'content-type': 'application/json' },
		})
	} catch (err) {
